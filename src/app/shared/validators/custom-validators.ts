...
export class CustomValidator {
    ...
    // 2 fields validators
    static lessThanValidator(targetKey: string, greaterKey: string): ValidatorFn {
        return (group: FormGroup): { [key: string]: any } => {
            const target = group.controls[targetKey];
            const greater = group.controls[greaterKey];
            const isMatch = +target.value <= +greater.value;
            if (!isMatch && target.valid && greater.valid) {
